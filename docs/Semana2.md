...
Implementada a lógica de agrupamento de chamadas de sistema (syscalls) em `src/student/pairer.c`. As informações da entrada da syscall são salvas no estado do pareador e, ao receber a saída correspondente, as duas partes são combinadas em um único evento completo.

**Implementação 2 (NOME): `launch_tracee`**
...
**Implementação 3 (NOME): `wait_for_initial_stop`**
