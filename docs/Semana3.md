...
Lógica que permitirá prosseguir a execução do processo monitorado utilizando `ptrace(PTRACE_SYSCALL, ...)`. O objetivo é deixar o processo filho executar livremente até encontrar a próxima entrada ou saída de uma chamada de sistema, repassando eventuais sinais pendentes.
